...
// After any install, compare OTHER bundles' installed hook scripts byte-for-
// byte with THIS package's current copies. Installing bundle A never touches
// bundle B's hooks — correct ownership-wise, but it silently strands B on
...
export function checkSiblingBundleHooks(targets, cwd = CWD, pkgRoot = PKG_ROOT) {
  ...
  for (const t of targets) {
    ...
    for (const e of entries) {
      ...
      const pkgScripts = join(pkgRoot, "bundles", e.name, "hooks", "scripts");
      if (!existsSync(pkgScripts)) continue; // not a bundle-owned dir (core hooks etc.)
...
async function main() {
  ...
  // Other bundles' hooks are never touched by this install — flag the ones
  ...
  try {
    for (const [id, files] of checkSiblingBundleHooks(targets)) {
      console.log(`  ℹ bundle '${id}': installed hooks differ from this package's version (${files.join(", ")}) — a stale fix or a local patch; refresh with: init --bundle ${id} --update`);
    }
