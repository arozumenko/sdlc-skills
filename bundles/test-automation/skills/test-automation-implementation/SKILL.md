...
2. **Doc-sync pass** — walk your branch diff for every selector, expected value, or step your implementation changed or discovered, and check each against the paper it came from: the AFS rows (Concrete Handles, Test Steps prose) state the SHIPPED truth, not the analyst's pre-drift version; the `_surface` digest carries your appended facts and no claim your code now contradicts; any case-text divergence is FILED as a clarification per the project's bug-filing style (§ Reverse-masking guard). The pass extends to whatever cross-references THIS project's seeded conventions define — the seed (`.agents/testing.md`, `.agents/profile.md`), not this skill, says which exist: where the project maps case priority to test markers, the marker must match the AFS; where tests carry tracker/TMS links, each must resolve to a real artifact, never a guessed slug. And **re-run this pass after every fix round** — a fix that adds a page-object method or renames a handle re-creates the same debt, and reviewers catch exactly that ("added in fix round 1, not reflected in the AFS"). These are obligations Phase 2 and Rule 11 already created — this step is where they are **checked**, because the reviewer's third lens triangulates case ↔ AFS ↔ diff row by row. Field-measured twice: 4 of 5 blocking findings on two small batches (2026-08-17/18), and **78 of 150 blocking findings (52%) across 112 field reviews** on a mature project — stale paper, not test code, is the single biggest reason a unit fails review; every skipped sync buys another review/fix round.
