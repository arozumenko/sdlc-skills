...
test-automation engagement, name withheld (this factory is public; see
...
manual-qa factory's `test-sizer` rubric has priced exactly that axis for
...
has drifted from the live product. Each predictably costs engineer time
(re-deriving intent at build) and raises rework odds (drift discovered
...
- **Operating shape — size is not the driver; dispatch mechanism is
  ...
  **But bigger is not better.** The cheapest run was a 13-case *sequential*
  ...
  more individual gate/merge passes plus 6 blocked cases that burned the
  full per-case loop before landing.
- **Delivery rate & the blocked-case premium.** Blocked cases cost **~1.85×
  the median delivered case** (they burn the full build+review+gate loop,
  then don't land); in the measured window 19% of all spend bought
  ...
  $/case-delivered (what a shipped test costs) — and states the assumed
