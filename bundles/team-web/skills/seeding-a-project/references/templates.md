...
- [.agents/quality.md Template](#agentsqualitymd-template)
...
**OPTIONAL — generated by Step 6.6 only on QA-heavy projects** (see
SKILL.md). Read on demand by the quality-architect (Quinn) when running
`quality-audit-workflow` and its specialist audits — **never** added to
the hook's always-injected shared-doc set. Scout derives every field
from the codebase and stack it already surveyed; **no user interview**.
...
> Read on demand by the quality-architect before audits. Update when
