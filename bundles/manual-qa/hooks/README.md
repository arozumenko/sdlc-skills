...
> **Coexistence with other bundles.** These hooks match on the *tool*
> (`Agent`), so in a repo that also runs another bundle (test-automation,
> feature-development) they would fire for every bundle's dispatches. A
...
> bundle's** (app-profiler / test-sizer / test-author / test-run-lead /
> test-runner / test-reporter) — other bundles' sessions get no state files,
