...
- **Bundle-owned skills (mirrored from global)** — [`skills/xlsx-reader/`](skills/xlsx-reader/), [`skills/playwright-testing/`](skills/playwright-testing/) — real copies of the canonical `skills/<name>/` content, regenerated by `npm run sync:bundles`. Declared in `localSkills` so the bundle install copies them from this dir; canonical source stays in the global `skills/` catalog so they remain reachable via `--skills <id>` standalone. Never hand-edit these copies — edit the canonical file and re-sync.
- **Briefings** — _(none)_.
- **Hooks** — _(none)_.
