# Finding taxonomy — the `Finding.class` enum with CWE anchors

The machine contract is the `class` enum in
`security-evidence/references/finding.schema.json`: **fifteen values, closed**.
A claim with any other class is `claim-invalid` at `gate`. This page says what
each class means, which CWE anchors it, whether the class is a **data-flow
class** (typed `source|sink|control` citations required, see SKILL.md), and
what it is *not*, so two reviewers file the same defect under the same class.
...
| `unmapped` | no | Used by `ingest sarif` for scanner rules the mapping cannot classify. **A reviewer never writes `unmapped`**: if none of the fourteen classes fits, the finding is not a security finding for this bundle. | — | — |

## Data-flow classes

`injection`, `xss`, `ssrf`, `path-traversal`, `deserialization` are the five
data-flow classes (`DATA_FLOW_CLASSES` in the scripts; `gate` sets
`requires_typed_citations: true` on every finding of these classes). For them a
claim cites the **source** (where the untrusted value enters), the **sink**
(where it is consumed dangerously) and every **control** on the path that was
examined and found insufficient. Without a source and a sink the claim is not
a data-flow finding — it is a suspicion, and `refutation-criteria.md` says
...
`confidence` (0–10) is how sure the reviewer is that the cited code has the
defect **as described** — not how severe it is. Below 5, the refutation
criteria expect the claim to say what would raise it (`prerequisites`).
