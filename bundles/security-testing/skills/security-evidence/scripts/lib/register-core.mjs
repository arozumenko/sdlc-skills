...
//   <st>/register/finding-alias.jsonl TASK-029
...
function openLocked(ctx) {
  ...
  return { paths, engagement_id, events, projection, rebuilt };
}

/**
 * Open the register: create the directory, run the recovery rule (under the
 * lock), return the verified log and the current projection.
 * @param {object} ctx
 * @returns {Promise<{paths: object, engagement_id: string, events: object[], projection: object, rebuilt: boolean}>}
