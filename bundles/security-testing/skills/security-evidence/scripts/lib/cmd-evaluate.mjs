...
// command line, no clock. Prints `evaluate()` of the payload as one JSON line
 ...
 * @param {{out: (line: string) => void, log: (line: string) => void}} ctx
...
export async function run(argv, ctx) {
  ...
  if (positional.length === 1) file = positional[0];
  ...
  if (rawText !== undefined) {
    ...
    try {
      ...
      if (err instanceof CanonError) return fail(`${file}: ${err.message}`);
      if (err instanceof IntegrityError) {
        ...
        if (err.expected === undefined) return fail(`${file}: not a verify artifact: ${err.reason}`);
        ctx.log(`evaluate: ${err.message}`);
        return EXIT_INTEGRITY;
      }
      if (err && err.code === "ENOENT") return fail(`${file}: no such file`);
      if (err && (err.code === "EISDIR" || err.code === "EACCES")) return fail(`${file}: ${err.code}`);
      throw err;
    }
