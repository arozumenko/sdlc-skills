...
//
//   loadSource(runDir, {command}) → Source
//
//   Source = {
//     run_id, dir, template,
...
//   }
//
// Reads go through inputs.pathGuard over the run directory alone, so a
// source can never pull in another run, the live register or the drop-box
// (G-16's boundary, applied to publication). Checks, in order:
...
//   run.json / manifest.json / claimed / gate-result / an import record that
//   is not the artifact it claims to be, or not this run's
//                            ⇒ 5 INCONSISTENT(<file>)
...
// classes), ../exit.mjs, ../inputs.mjs (pathGuard — the file-system door),
// ../ledger.mjs (RUN_ID), ../tokens.mjs. No writes, no child process, no git,
// no clock.
...
/** @typedef {{run_id: string, dir: string, template: string, run: object, manifest: object, report: Buffer, claimed: object | null, gateResult: object | null, imports: object[]}} Source */
 ...
 * @param {{command: string}} options the calling command, for the exit-2 tokens
 * @returns {Source} frozen
 * @throws {CliError} 2 USAGE · 5 INCONSISTENT(<file>)
 */
export function loadSource(runDir, { command } = {}) {
  ...
  return Object.freeze({ run_id, dir, template: manifest.payload.template, run, manifest, report, claimed, gateResult, imports: Object.freeze(imports) });
}
