...
// Review fix: `matchMergeSubject` searches ALL of `run.items`, unfiltered — so before this fix a
// cancelled task (task/task-002's branch is genuinely merged by buildFixtureRepo) still produced a
// `done/episode-1` record. `deriveGitObservations` must apply the same eligibility filter used for
// `created`/`first_commit`/`rework_observed` (level === 'task' && !cancelled) to the `done` match too.
test('review fix: a cancelled task whose branch is still merged yields no records at all — not even done', () => {
  const { repo, head } = buildFixtureRepo();
  const cancelledRun = run(repo);
