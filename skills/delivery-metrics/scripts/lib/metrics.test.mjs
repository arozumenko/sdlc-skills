...
test('computeMetrics: hand-computed flow, throughput, estimates (rows, strata, exclusions, band), coverage, rework', () => {
  ...
  // F17: quality denominators also per class stratum, not only per level. S = t1,t2,t7,t8 (4); M = t3,t4,t5 (3); L = t6 is WIP not done (0).
  ...
  // first_commit (first_commit_source): only t1 and t2 have a commit — t2 isn't in the done-task set? t2 IS done. t1, t2 both have commit set.
