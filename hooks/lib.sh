...
role_memory_files() {
  ...
  # `__none__` (either level) = no memory files from the hook. The SOUL gate
  # below applies to override lists too — it encodes host mechanics (who expands
  # @-imports), not preference, so an override cannot re-break the duplication.
