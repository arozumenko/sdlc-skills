# delivery-metrics — design spec (v3)
...
**Status:** v3 — round 2 `needs-changes` addressed: 7 resolved (0 blockers, 6 majors, 1 minor), 0 rejected; no new narrowing; ready for round 3 (§19.2).
...
| `direct_active_min.v1` | `active_min`, automation case/mission | Sum quoted case direct.activeMin / mission totals.activeMin from a fresh final bound export; accounting window begins at bound scope declaration and ends at first qualifying case completion / mission landing. Wrapper must prove contributing dispatches belong to this window/generation, else `boundary-unknown`. No loaded allocation in accuracy. |

`direct_active_min.v1` additionally requires `actual_window: {scope_binding_id, generation, start: "scope_declared", finish: "first_case_done"|"first_mission_landing", item_id}` in the estimate digest. Bind the stable scope identity before declaration; future endpoints are named selectors, not fabricated timestamps. Every included dispatch must start at/after declaration and end at/before the selected finish; cross-boundary or unknown dispatch clocks exclude accuracy, never prorate quoted minutes. Window ids/endpoints are not inferred from unit or receipt mtime. Timely acceptance is strictly before the named start (and cutoff), for original and latest policies. Missing start/finish/binding, commit/gate/scope completion proxies or unknown accounting window exclude accuracy with reasons; never substitute commit_to_done for cycle_time. Parents without descendant observed starts are missing-actual. A different `actual_metric`, window or definition requires renewed acceptance. Unbound legacy estimates remain visible as `unbound-actual`; group comparisons by metric version, unit, level and basis. No pooling lead/cycle/parent/active ratios.
...
skills/delivery-metrics/
  ...
  scripts/lib/transactions.mjs              + test (byte recovery, reconcile, participant lock/sync)
...
| M3 | Automation | bound plan/scoring/outcomes, required-N gate proof/landing, snapshot-only batch/case cost computation and atomic provenance bundle, drift retry, calibration/reference log and playbook; no raw/task-cost reader |
...
| Accepted ranged inputs, valid zeros D9/D12/D22 | §6.3–6.4/6.10/9 → schema/templates/metrics/calibration → v2 F5 actual-boundary tests, AC-10/13 |
| Separate episode age D7/D22 | §6.9–6.11 → timeline/metrics/report/calibration M1 → v2 F6 tests, AC-18 |
| Shared participant compatibility | §6.1/6.12 → §10 amendments M-1/M3 → v2 F4 concurrency/provenance tests, AC-14/16 |
