...
| Owner | Follow-up from review |
|---|---|
| TASK-006 (ctx.mjs, G-15) — before `publish --to` (TASK-025) | `resolveOut` in `cmd-register-render.mjs` re-implements `ctx.input()`'s cwd-realpath + root-containment rule with a `cannot write` message; `publish --to` needs the same. Add a `ctx.output(command, p)` twin in `ctx.mjs` (containment only — the G-5 prefix / reserved-set policy stays with the command) and have `render` and `publish` both call it, so one containment rule exists. |
