...
### PM log additions (after G27)

| Owner | Note |
|---|---|
| TASK-049 follow-up (manual-qa doc pass: test-run-lead Step 2 clause) | TASK-049: bundles/manual-qa/agents/test-run-lead/AGENT.md Step 2 ("Size Unsized Cases") has no back-reference to the Step 0 read-only rule: a lead that lands on Step 2 without re-reading Step 0 dispatches `test-sizer` on the admitted suite and the `size:` Edit changes the file's redacted-bytes identity, so `sign-off` lists it as UNADMITTED and `plan.mjs ta-prompt` refuses it. One clause in Step 2 ("unless Step 0 routed a security admitted suite — run it unsized") closes the gap. Owner: a manual-qa doc pass (TASK-049 follow-up); non-blocking because Step 0 precedes Step 2 and the row is explicit. |
| TASK-042 (security-test-planning case template `size:`; PM to route) | TASK-049: Dev's routed nit, confirmed: neither `security-test-planning` nor `profiles/case.mjs` emits `size:`, so every admitted case arrives unsized. `case.mjs` keeps every frontmatter key in order, so a `size:` line authored in the candidate would survive publish unchanged — the cheapest fix is the security-test-planning case template carrying `size:` (owner TASK-042), not an admit-time requirement (TASK-043). PM to route. |
| TASK-052 (gen:marketplaces) — closed by G27 merge | TASK-049: `node bin/gen-marketplaces.mjs --check` reports `.github/plugin/marketplace.json` stale — pre-existing on feat/security-testing-bundle-spec (verified at 6af6bb0), not introduced by this diff (the agent `description:` is untouched). Owner: TASK-052 (`npm run gen:marketplaces`). |
| TASK-049 (note only: no new test) | TASK-049: No new *.test.mjs — accepted. The change is agent prose; a test asserting a prose bullet is tautological, the task's Verification pins `git diff --stat` to one file, and G-15 keeps the manual-qa test files out of this task's ownership. Red/green was the row-presence grep (1 match on the branch, 0 on base). |
| PM (test-automation code-review copy: one-line PR or new dupe group) | TASK-050: Cross-factory copy: bundles/test-automation/skills/code-review/SKILL.md is byte-identical to the feature-development one on base but is not a check-skill-dupes.mjs group, so it stays un-pointed. gen-marketplaces sources code-review from the alphabetical-first owner (feature-development), so the generated catalogs pick up the pointer; a `--factory test-automation` install does not. Route to PM: either a separate one-line PR to the test-automation copy, or a new dupe-group entry pairing the two. US-044 AC-1 names only the feature-development file, so it is out of this task's scope — correctly left alone. |
| TASK-050 follow-up (optional: self-contained verify.mjs path) | TASK-050: The bullet spells the fix-verification command as bare `verify.mjs all …` while secure-code-review § Standalone step 12 spells it `node <scripts>/verify.mjs all …`. As a pointer it is fine (the reader lands in secure-code-review, which gives the path); if the PM wants it self-contained, a follow-up could add `<skills dir>/security-evidence/scripts/`. Not blocking. |
| TASK-052 (validate:marketplaces) — closed by G27 merge | TASK-050: validate:marketplaces reports three stale manifests — pre-existing on feat/security-testing-bundle-spec (identical 3 stale on base), owned by TASK-052 per the TASK-048 deviation record; untouched here, as routed. |
| TASK-050 (note only: skills-ref CI-only) | TASK-050: skills-ref could not be run locally (not installed; CI-only pip install). Verified instead that the diff touches no frontmatter (0 `---` lines in the diff), so the agentskills.io spec check on `name`/`description` is unaffected; CI will confirm. |
| TASK-052 follow-up / TASK-053 (README.md:21 standalone-install wording) | TASK-052: bundles/security-testing/README.md:21 — "the two-skill standalone install (no agents) runs the full command set except the threat-model and planning commands, which need their agents": tm-lint.mjs / plan.mjs are in security-evidence and do run in that install; what is missing is the agent-authored input (a threat-model.json, cases), not the commands. Reword to "…runs every command; the threat-model and planning commands have nothing to lint or admit without their agents". Owner: TASK-052 follow-up or TASK-053 (it rewrites the install section anyway). |
| PM note (catalog.test.mjs:77 repo-wide staleness coupling) | TASK-052: bundles/security-testing/skills/security-evidence/scripts/catalog.test.mjs:77 — the `gen-marketplaces.mjs --check` test makes the whole `npm test` red whenever any factory in the repo adds a skill without regenerating (it embeds `validate:marketplaces` inside the security bundle's suite). AC-2 asks for exactly this, so it stays; note for the PM that the failure message will point at security-evidence for a repo-wide staleness. Same repo-root coupling as the manifest.test.mjs precedent (REPO_ROOT resolve) — harmless in this repo, meaningless in a consumer install. |
| TASK-006 lineage (cli.mjs:105 non-EPIPE rethrow, G-4) | TASK-052: bundles/security-testing/skills/security-evidence/scripts/lib/cli.mjs:105 — the rethrow inside the `once('error')` listener surfaces a non-EPIPE write error as an uncaught exception from Node's nextTick emit, which prints an unredacted stack (G-4). Identical to the pre-change behaviour (an unhandled 'error' event did the same), so no regression; if a later task wants stderr fully redacted it would `complain(redactString(err.message))` and set exitCode 1 there instead of rethrowing. Route: cli.mjs owner (TASK-006 lineage), not this task. |
| PM (flaky test; G27 integration run 1891/1891 captured to file, no failure) | TASK-052: One flaky failure (1/4 full-suite runs: 1890 pass / 1 fail, name not captured because the first run was piped through grep) — not reproducible: 3 further full runs = 1891/1891, cli.test.mjs 15/15, catalog.test.mjs 5/5. Almost certainly a pre-existing timing-sensitive test elsewhere in the suite, not this diff; PM may want the next full-suite run captured to a file to name it. |
| TASK-047 follow-up / TASK-054 (confirmed untouched) | TASK-052: skills/security-engagement/references/workflow.md step 2 M1-dated sentence and security-lead AGENT.md:396 INCOMPLETE causes remain routed to TASK-047 follow-up / TASK-054 as the dev recorded — confirmed untouched here, as they should be. |
