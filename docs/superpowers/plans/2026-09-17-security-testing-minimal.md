...
### PM log additions (after G4)

Reviewer nits from Rio's PASS reviews of Task 5 and Task 6, recorded verbatim. Owner = the task that must act on the nit.

| Owner | Source | Nit |
|---|---|---|
| Task 2 (rule) / PM decision, before Task 7 pins the copies | Task 5 | Task 5: [redaction-rules.json:9 high-entropy + cite.mjs redactDeep → owner Task 2 (rule) / PM decision BEFORE Task 7 pins the byte-identical copies; Task 5 only inherits it] The high-entropy rule `[A-Za-z0-9+/]{40,}` matches any slash-separated path of 40+ chars. Reproduced on bed6466 with scope `src/` and `src/main/java/com/example/service/impl/UserServiceImpl.java`: `check` exits 0 but writes back `citations[0].path` and `coverage.rows[].path` as `<REDACTED:high-entropy>.java`; the re-run then prints `USAGE(check: examined: not in scope: <REDACTED:high-entropy>.java)` exit 2 — D2 re-runnability is broken for any project with such paths (Task 4's merged write-back, not this diff), and `--md` prints `\| <REDACTED:high-entropy>.java:2-3 @… VERIFIED \|` into the tables the lead pastes. Fix options for the PM: (a) rule: negative lookahead that excludes runs with two or more `/`-separated word segments, e.g. `(?<![A-Za-z0-9+/])(?!(?:[A-Za-z0-9]+/){2})[A-Za-z0-9+/]{40,}={0,2}(?![A-Za-z0-9+/=])`; or (b) `redactDeep`: a PATH_KEYS set (`path`) whose values are kept when `canonicalPath(v) === v`. Either keeps id preimages unchanged (findingId hashes `first.path` unredacted already). |
| Task 7 / Task 5 (optional hardening) | Task 5 | Task 5: [cite.mjs:461-470 admittedCases CASE_PREFIX `^(TC-\d{3})_` → owner Task 7] `.admitted.json` `file` must be the bare basename `TC-NNN_<slug>.md` (as the plan's Task 5 test comment and Task 7 Step 3 spell it); a suite-relative path (`tasks/security-acme-admitted/TC-001_x.md`) would make every `planned(TC-nnn)` disposition `TM-INVALID … not in the admitted suite`. Optional one-line hardening in Task 5's reader: `CASE_PREFIX.exec(basename(entry.file))`. |
| Task 6 | Task 5 | Task 5: [cite.mjs:520-541 registerRows fold → owner Task 6] Task 5 treats a row as live once an `add` event names it unless its latest event is `supersede` or `close-false-positive` (`fixed`, `accepted`, `regressed`, `reopen`, `ticket` keep it live; `reopen` after `close-false-positive` revives it). Task 6's `foldEvents`/`supersede` must agree: the supersede event must be appended under the SUPERSEDED row's `row_id` (`supersede R-a --by R-b` ⇒ `{row_id:"R-a", event:"supersede"}`), and a `fixed` row must still count as 'present in the register' for `accepted(R-nnnn)`, or the two folds silently diverge. Event shape `{seq, ts, actor, row_id, event, payload}` and `CORRUPT events.jsonl:<n>` (exit 5) already match Task 6's plan text. |
| Task 6 / Task 2 | Task 5 | Task 5: [cite.mjs:73-93 CorruptError local to cite.mjs → owner Task 6 / Task 2] The G1-logged cli.mjs `exitCode` mapping is still unimplemented; Task 6 will need its own CorruptError → 5 path (a local `command` wrapper like Task 5's, or the cli.mjs mapping landed before Task 7 pins the copies). No divergence in Task 5. |
| Task 2 (pinned by Task 7) | Task 5 | Task 5: [lib/redact.mjs redactString hits → owner Task 2 (pinned by Task 7)] `hits` counts a rule re-matching an earlier placeholder (`token = <REDACTED:key-value>` satisfies the key-value rule again). Task 5's `redact` reports hits=0 when the bytes are unchanged, but a file with one existing placeholder plus one new secret reports `hits=2` (verified: `redactString("token = <REDACTED:key-value>\npassword = hunter22xyz\n").hits === 2`). Informational line only; fix in the lib is to not count a match whose replacement equals the matched text. |
| Task 9 / Task 12 (docs) | Task 5 | Task 5: [Task 9 / Task 12 docs, from the dev's own list, confirmed] Document: `CORRUPT events.jsonl:<n>` and `CORRUPT tasks/security-<slug>-admitted/.admitted.json` (exit 5, nothing written); the threat-model FAILED locus form `FAILED E-001.0 <why>` / `FAILED M-001.0 <why>` with fallbacks `elements[i]` / `threats[i].mitigations[j]` when the id is unusable; `USAGE(check: --no-snippets needs --md)`; `USAGE(check: neither a findings nor a threat-model document …)` for a file with none of findings/elements/threats (an empty `{elements:[],threats:[]}` model is valid: `MODEL elements=0 threats=0 open=0`); under `--md` the `CHECK` line is no longer last — the tables follow (one blank line apart, no headings) and `TABLES sha256=<hex>` is last, hashing exactly the printed block (whole-text redacted, trailing newline included); `--md` tables are printed on exit 4 too. |
| Task 9 / Task 10 (docs) | Task 5 | Task 5: [Task 9 / Task 10 docs] Threat-model D10 is per position: `E-nnn`/`T-nnn`/`M-nnn` `id` at element/threat/mitigation positions is the agent's; `state`/`verdict` anywhere and `id` at any other position (a citation, top level, a nested object such as `threats[i].meta.id`) is `REFUSED agent-written key <key>` — verified on bed6466 for all of those. The threat-modeler prose must say the model file is written without `state`/`snippet_redacted`/`check_stamp` and that a FAILED citation is fixed by rewriting the unstamped source (same loop as findings mode). |
| PM (test-line budget decision) | Task 5 | Task 5: [PM: test-line budget] The bundle now carries 1,543 test lines against spec §10's ≤2,000, with `register`, `cases`, `verify` and their lib tests (Tasks 6–8) still to come; the E2E lives in bin/ and may be outside the count. Worth deciding now whether the bound is binding or advisory. |
| Task 5 (cosmetic, no action) | Task 5 | Task 5: [cite.mjs size, cosmetic] cite.mjs is 699 lines against spec §6's '~600' estimate; the growth is the header comment, JSDoc and the per-mode D10 table, not new behaviour. threat-model.mjs 129 (plan ~90) and md.mjs 100 (plan ~70) likewise. No module, reference or fixture beyond the plan's Task 5 list was added. |
| Task 5 (cosmetic, no action) | Task 5 | Task 5: [cite.mjs:571 locusOf, cosmetic] Any non-empty string `id` is used as the FAILED locus, so a malformed id prints e.g. `FAILED x.0 snippet-not-found` beside `TM-INVALID elements[0]: id must match E-nnn`; two elements sharing an id print two `FAILED E-001.0` lines plus `TM-INVALID E-001: duplicate id`. Readable, no action. |
| Task 6 (cheap) | Task 6 | Task 6: [lib/transitions.mjs:48 ROW_ID + register.mjs:140 nextId → owner Task 6, cheap] Row-id rollover self-corrupts the log: verified with a fabricated 9999-add log that `add` #10000 exits 0 printing `ROW R-10000 open`, after which every command exits 5 `CORRUPT events.jsonl:10000 row_id is not R-nnnn` — the CLI writes a line its own fold refuses, contradicting foldEvents' docstring invariant. Fix: `ROW_ID = /^R-\d{4,}$/` (transitions.test.mjs:104 `R-1` still throws) or have `add` refuse when `!ROW_ID.test(nextId(rows))`. |
| Task 6 (optional) / Task 2 | Task 6 | Task 6: [register.mjs:342 FINGERPRINT regex → owner Task 6 (optional) / Task 2] engagement.mjs only requires `engagement_id` non-empty, so an id containing `:` (verified `acme:2026`) makes `status --expect <its own FINGERPRINT>` print `USAGE(status: --expect must be a FINGERPRINT …)` exit 2 — `[^:]+` cannot split it. Parse from the right: `/^(.+):(\d+):([0-9a-f]{64})$/` (the tail is fixed-shape, greedy `.+` is safe); or Task 2 constrains engagement_id to exclude `:`. |
| PM decision / Task 11 (docs) | Task 6 | Task 6: [register.mjs:42-44 EXPOSED/APPROVED → owner PM decision, dev flagged] UNAUTHENTICATED-APPROVALS counts rows in accepted\|false-positive while OPEN-EXPOSURE counts open\|regressed\|accepted — so `close-false-positive`, which records an approval-like payload, does remove a row from exposure (verified: p2=1 → p2=0). Spec §2 row 6 says open exposure is never reduced by an approval; the PM must confirm a false-positive closure is a disposition rather than an approval (then the docs say so) or add `false-positive` to EXPOSED. Task 11's risk-register SKILL.md must state whichever is chosen. |
| Task 11 (docs) | Task 6 | Task 6: [register.mjs:166-188 transition → owner Task 11 docs] Check-then-append TOCTOU: two concurrent invocations both fold seq N and both append seq N+1; the next fold exits 5 (seq gap check). Single-operator CLI, no lock in scope; the risk-register SKILL.md should say one writer at a time and that a duplicate-seq log is repaired by deleting the later line. |
| Task 6 (optional) | Task 6 | Task 6: [register.test.mjs:287 → owner Task 6, optional] The test titled 'fixed/regressed are exported for verify.mjs and never print' never asserts silence; capture `process.stdout.write` around `mod.fixed(...)`/`mod.regressed(...)` and assert nothing was written so the test body proves its title. |
| Task 2 (already in the PM log) | Task 6 | Task 6: [cli.mjs:102 → owner Task 2, already in the PM log] `register status --help` prints `USAGE(register: --help needs a value)`; the dev correctly left cli.mjs byte-identical. No action for Task 6. |
| Task 11 (docs), note for Task 8 | Task 6 | Task 6: [register.mjs:210 → owner Task 11 docs, note for Task 8] A `--verify` path outside the root is stored as the absolute path (verified: `/var/folders/…/verify.json` in payload.verify), embedding a machine path in the shared log; Task 8 always passes `<st>/verify/<id8>-<oid7>/verify.json` so the in-process path is fine — docs should require the verify path to live under `<st>/verify/`. |
| Task 11 (docs) | Task 6 | Task 6: [register.mjs:317 check → owner Task 11 docs] `check` expires when `accepted_until < today` (UTC), so an acceptance `--until` today is not expired until tomorrow UTC (verified: prints nothing). Docs should say 'expires the UTC day after --until'. |
| Task 7 / PM (plan) | Task 6 | Task 6: [Task 7 / PM plan] Consequence of the blocking fix: Task 7 Step 4's redact dupes group must pair secure-code-review's `lib/{redact.mjs,redaction-rules.json}` with BOTH security-test-planning and risk-register (three-way), and the plan's file-structure line '`lib/redact.mjs` + `redaction-rules.json` across two' becomes 'across three' with risk-register's lib list gaining the two files. |
| PM (open item) | Task 6 | Task 6: [PM open item, unchanged] D5 first-line collision (two findings at two locations → one id) is still undecided; `add` keys the live row on the full 64-hex id as the plan says. Task 6 is agnostic to how the id is computed, so the PM decision needs no Task 6 change either way. |
| PM (close the G1 nit) / Task 7 | Task 6 | Task 6: [G1 PM-log nit cli.mjs:112-118 → can be closed] Exit 5 for CorruptError is mapped inside register.mjs's own `command` wrapper (verified: corrupt log → `CORRUPT events.jsonl:3 …` exit 5, no write); no `e.exitCode` addition to cli.mjs is needed and cli.mjs/engagement.mjs are byte-identical to the merged Task 2 state (diff empty), so Task 7 can pin them as-is. |
| PM (accepted deviations, no action) | Task 6 | Task 6: Dev deviations reviewed and accepted as spec-consistent: `--expect` accepts the pasted line with or without its `FINGERPRINT ` prefix (verified MATCH both ways); `supersede R-a --by R-b` treats `--by` as the target with actor `$USER` (spec §6 spells it so); `--authenticated` on any verb is USAGE; `revoke`/`close-false-positive` carry the four APPROVAL_KEYS with `authenticated:false` and defaults; `add --finding` requires the full 64-hex id; `check` prints only `EXPIRED <id>` lines (spec §6 names no summary). The task text's 'nothing merged' is stale — feat/security-testing-minimal carries Tasks 1–4 at 3428e0c, confirmed. |
| Task 6 (before Task 8 consumes) | Task 6 | Task 6: [register.mjs:394-399 `cell`, :380 `status --json` → owner Task 6, recommended before Task 8 consumes] `render` and `status --json` re-emit folded row strings (title, owner, ticket_url, approved_by, approval_ref, actor) without a redaction pass; only the append path redacts. Reproduced: a hand-appended log line `{"event":"ticket","payload":{"ticket_url":"https://x/?token=hunter22222"}}` folds fine (structure-only validation) and `render` writes `token=hunter22222` into `risk-register.md`; `status --json` prints it. cite.mjs's precedent is belt-and-braces (`redactDeep(doc)` on write-back). Cheap fix: `cell = (v) => clean(...)` and a field-wise `clean` over the `--json` document keeping `finding_id`/`sha256`/`fingerprint` intact (the 64-hex tail would otherwise be eaten by the high-entropy rule). PM may promote this to blocking; I kept it a nit because the trigger is a hand edit of a script-owned file, not a CLI path. |
| Task 6 | Task 6 | Task 6: [register.mjs:137 CorruptError from TransitionError → owner Task 6] `CORRUPT events.jsonl:<n> <why>` echoes a log-line KEY name unredacted via transitions.mjs:83 (`payload key ${k} is not a field …`): a hand-edited line with key `AKIAAAAAAAAAAAAAAAAA` prints `CORRUPT events.jsonl:3 ticket: payload key AKIAAAAAAAAAAAAAAAAA is not a field ticket sets` (verified, exit 5). Contradicts the header comment 'Messages never echo log bytes'. Wrap with `clean(e.message)` at line 137 or drop the key name from the message. |
| Task 2 (engagement.mjs shape) + Task 11 (docs) | Task 6 | Task 6: [register.mjs:351,423 FINGERPRINT / render `- engagement:` / status --json → owner Task 2 (engagement.mjs shape) + Task 11 docs] `engagement_id` is printed and written unredacted; engagement.mjs only requires a non-empty string. cite.mjs never emits it, so this is the first script to do so. Redacting it would break `--expect` MATCH, so constrain the shape in engagement.mjs (e.g. `^[A-Za-z0-9._:-]{1,64}$` — short enough that the high-entropy rule cannot match) or document it as the operator's own config value. Note the `--expect` regex already tolerates `:` in the id (test at :364). |
| Task 6 (one-liner) / Task 8 / Task 11 (docs) | Task 6 | Task 6: [register.mjs:214 `resolve(root, verifyPath)` → owner Task 6 (one-liner) / Task 8 / Task 11 docs] A relative `--verify` path resolves against the repo ROOT, not the invoking cwd (`ctx.cwd` is available but unused), unlike every other relative-path CLI convention; running from a subdirectory silently looks in the wrong place (`USAGE(fixed: no such file …)`). Either resolve against `ctx.cwd` in `verifyVerb` (keep root for the in-process export) or state 'root-relative' in the SKILL.md and in Task 8's call site. |
| Task 8 / Task 6 (optional) | Task 6 | Task 6: [register.mjs:227-230 exported `fixed`/`regressed` → owner Task 8] `rowId` is not shape-checked in the in-process path; a bogus id is refused as `RefusedError("no row <raw id>")` with the raw id in `.token`/`.message` (no write happens, so not a leak here). verify.mjs must redact what it prints from these errors, or Task 6 adds `if (!ROW_ID.test(rowId)) throw new UsageError(...)` in `verifyEvent`. |
| PM (plan File structure) / Task 7 | Task 6 | Task 6: [plan File structure lines 46 and 57 → owner PM] risk-register now ships `lib/redact.mjs` + `lib/redaction-rules.json` (cp'd byte-identical, cmp-verified by me: same blob ids as secure-code-review and as the merged base) per the review-1 blocking fix; line 46's `scripts/lib/{transitions,engagement,cli}.mjs` and line 57's 'across two' must become `{transitions,engagement,cli,redact}.mjs` and 'across three' before Task 7 writes the dupes group. Nothing else outside the plan's named files exists in the diff (8 files, all under skills/risk-register/scripts/; cli.mjs/engagement.mjs unchanged from Task 2's blobs). |
| Task 6 (optional) / PM note | Task 6 | Task 6: [size → owner Task 6 (optional) / PM note] register.mjs is 497 lines against spec §6's ~350 and the plan's ~300; transitions.mjs 172 against ~110. No unnamed module, reference or fixture, and tests total 498 lines (well under the ≤2,000 budget), so not blocking — but the two test files plus lib are 1,167 lines for one script; the PM should keep the ~350 target in view when Task 8 reuses these exports. |
| Task 13 (already logged) | Task 6 | Task 6: [validate:marketplaces → owner Task 13, already in the PM log after G1] `.cursor-plugin`, `.codex-plugin` and `.github/plugin` report stale on this branch; pre-existing (nothing in this diff is read by gen-marketplaces.mjs). validate:factories and validate:dupes are green; validate:externals not run per instruction. |
| PM decisions / Task 11 (docs) | Task 6 | Task 6: [PM decisions the dev correctly did not act on, restated so they are not lost] APPROVED=[accepted,false-positive] means `UNAUTHENTICATED-APPROVALS` counts by current status (a revoked acceptance counts 0 although its approval record remains) — spec-compatible, PM to confirm; `check` uses the UTC calendar day (`today()` at :323) — Task 11 docs; D5 first-line id collision (G3 log) still open before `add` treats the id as the live-row key. |
| PM (plan lines 46/57) / Task 7 Step 4 | Task 6 | Task 6: [PM (plan File structure lines 46/57) / Task 7 Step 4] risk-register now carries a third copy of lib/redact.mjs + lib/redaction-rules.json (required by the Global Constraint 'redaction on every write', not named by the plan's risk-register line). Verified byte-identical to Task 2's (blobs ac9f787 / 8191d05; cli.mjs c7bdd36 and engagement.mjs 6edb581 likewise). Task 7's dupes group for redact.mjs/redaction-rules.json must include risk-register's copy, and the PM should amend 'across two' to 'across three' so the copies cannot drift. |
| Task 6 (optional) | Task 6 | Task 6: [Task 6, optional] register.mjs `transition()` supersede guard (and the matching `foldEvents` check) only requires `payload.by` to exist: verified `supersede R-0001 --by R-0002` then `supersede R-0002 --by R-0001` both print ROW … superseded, leaving a two-row cycle of dead rows (COUNT superseded=2). The reference refused a superseded target; add RefusedError('supersede target R-nnnn is superseded') in both places. |
| Task 2 (engagement.mjs) / Task 11 (docs) | Task 6 | Task 6: [Task 2 engagement.mjs (pinned copy), docs Task 11] engagement_id is validated only as a non-empty string and register is the first script that prints it raw: verified engagement_id 'token=hunter22222' yields `FINGERPRINT token=hunter22222:1:<sha>` on stdout and `- engagement: \`token=hunter22222\`` in risk-register.md. Redacting it in register would break `--expect` MATCH, so constrain the shape in engagement.mjs (e.g. /^[A-Za-z0-9][A-Za-z0-9._:-]*$/) before Task 7 pins the copies. |
| Task 2 (redaction-rules.json) | Task 6 | Task 6: [Task 2 redaction-rules.json, extends the PM-log high-entropy entry] the high-entropy class `[A-Za-z0-9+/]{40,}` includes `/`, so a URL path of ≥40 alnum-or-slash chars is eaten: verified `ticket R-0003 https://github.com/o/r/commit/<40hex>` records ticket_url `https://github.<REDACTED:high-entropy>`, and an absolute --verify path echoed in `USAGE(fixed: no such file …)` prints `<REDACTED:high-entropy>_dr9…/T/…`. Plan-as-written; dropping `/` from the class would fix both. |
| Task 2 (cli.mjs) | Task 6 | Task 6: [Task 2 cli.mjs (pinned copy)] `USAGE(register: unknown command <sub>)` echoes argv raw (verified `register.mjs token=hunter22222`); cli.mjs `out()` does not redact by design and the same class exists in every script. Either redact `sub` there or drop the echo. |
| Task 6 (optional) | Task 6 | Task 6: [Task 6, optional] `foldEvents` does not re-apply the duplicate-live-finding rule: a hand-edited second `add` with the same finding_id folds cleanly and `rowForFinding` returns the first row. Consistent with the module's own definition (the CLI never writes one), but for symmetry with the `add must create R-nnnn` check it could throw CorruptError. |
| Task 8 | Task 6 | Task 6: [Task 8 note] In-process contract as implemented: `fixed(root, rowId, verifyPath, by?)` / `regressed(...)` → `{id, status}`, verifyPath resolved against root (absolute or root-relative; a path outside root is recorded absolute — verified). Task 8 must map finding id → row via the exported `readLog`/`foldEvents`/`rowForFinding(rows, finding_id)` (all exported and tested), and only call `regressed` when the row is `fixed` (the table refuses it otherwise with RefusedError). |
| Task 11 (docs) | Task 6 | Task 6: [Task 11 docs] `status --json --expect <fp>` prints the JSON document and then the MATCH\|ADVANCED\|DIVERGED line, so stdout is no longer one JSON document; docs should say to parse everything but the last line. |
| PM (test-line budget) / Task 6 | Task 6 | Task 6: [PM budget] Spec §10's ≤2,000 test-line budget: bundle test lines are 1,646 after Task 6 (register.test.mjs 407, transitions.test.mjs 125); Tasks 7, 8 and the prose tests of 9–12 will exceed it unless trimmed. register.mjs is 513 lines vs the ~350 target (dev flagged). |
| Task 13 (already logged) | Task 6 | Task 6: [Task 13, already logged] the three generated marketplaces are stale on this branch (gen-marketplaces --check prints 3 '! stale' lines, exit 0). |

### PM log additions (after G5)

Reviewer nits from Rio's PASS reviews of Task 7 and Task 8, recorded verbatim. Owner = the task that must act on the nit.

| Owner | Source | Nit |
|---|---|---|
| Task 7 (PM may promote) | Task 7 | Task 7 (PM may promote): [cases.mjs:142 `readdirSync(dir).filter(/^TC-.*\.md$/)`] verify-suite only inspects `TC-*.md`, so a hand-added `notes.md`, `tc-002_x.md` (lowercase) or a subdirectory in tasks/security-<slug>-admitted/ is silently ignored (verified: `SUITE ok=1`, exit 0, prompts printed). Plan Step 3 says 'list TC-*.md' so it is plan-as-written, but spec §2's script-enforced promise is 'contains only admitted cases; verify-suite lists anything else' and §6 says 'every file in the suite'. One-line fix: list every entry except `.admitted.json` (`readdirSync(dir, {withFileTypes:true})`), and any name not matching CASE_FILE or any directory ⇒ `UNADMITTED: <path>`. |
| Task 7 | Task 7 | Task 7: [cases.mjs:141-153] an `.admitted.json` entry whose file was deleted from the suite is neither reported nor pruned (verified: delete the admitted file ⇒ `SUITE ok=0`, exit 0), yet Task 5's `cite.mjs check` still accepts `planned(TC-001)` from the stale index entry — the two scripts disagree on what is admitted. Either prune absent entries on verify-suite (rewrite the index) or print `UNADMITTED: <path> (indexed, missing)` and exit 4. |
| Task 7 | Task 7 | Task 7: [cases.mjs:134,156-159] `headOid7` runs after `SUITE ok=` and both prompt lines are printed, so in a repo with no HEAD (before §7 step 1's first commit) the output is three result lines followed by `INTERNAL Error: Command failed: git rev-parse HEAD` on stderr, exit 1. Compute the oid first and map the failure to `USAGE(verify-suite: no HEAD commit)` (exit 2) so no partial hand-off is printed. |
| Task 7 | Task 7 | Task 7: [cases.mjs:46,79-82 `caseTitle`] HEADING runs over the whole file including the frontmatter, so a YAML comment line `# yaml comment` inside `---` becomes the TA-PROMPT title (verified: title="yaml comment" while the body heading is `# TC-009: real title`). Skip the frontmatter block before matching (the FRONTMATTER regex in admission.mjs could be exported or duplicated as one line). |
| Task 7 (low) | Task 7 | Task 7 (low): [cases.mjs:92-95] the `<st>/cases/` containment check is lexical (`resolve` + `startsWith`); a symlink `<st>/cases/TC-001_x.md → elsewhere/` is admitted (verified). Content is still redacted and linted so nothing leaks; `realpathSync` on the candidate would make the provenance check honest. |
| Task 7 (plan-as-written) | Task 7 | Task 7 (plan-as-written): [admission.mjs:132-137 `stepsTable`] the table must be the first non-blank line after `## Steps`; a prose line between heading and table ⇒ `USAGE(admit: ## Steps table is missing or empty …)` (verified). The manual-qa format has none, so acceptable; scanning forward to the first table within the section would be friendlier. |
| Task 7 (test hygiene) | Task 7 | Task 7 (test hygiene): [admission.test.mjs:36] the test title claims 'the shell payload an injection-payload hit' but the action `Inspect the response; then run \`rm -rf /\`` produces only `mutating-verb` + `tooling` (verified) and the test asserts only those — rename the test. [cases.test.mjs:152-153] the two `includes` asserts are subsumed by the regex on line 151. |
| Task 2 (already in the PM log, G4 line 106) | Task 7 | Task 2 (already in the PM log, G4 line 106): [redaction-rules.json high-entropy] the manual-qa prompt is whole-line redacted, so a `base_url` with a ≥40-char alnum/slash path prints `base_url=https://staging.example.<REDACTED:high-entropy>.` (verified); a 40+-char hyphen-free slug would likewise be eaten in the prompt line. Same class as the logged nit; Task 7 pins the rules as-is per the PM's G3 decision. |
| Task 11/12 (docs) | Task 7 | Task 11/12 (docs, from the dev's own note): knowledge/engagement.md.template line 24 says base_url is 'substituted for {{base_url}} in the hand-off prompts'; verify-suite carries it as `against base_url=<url>` and no prompt contains a `{{base_url}}` token. Trim the sentence to 'printed in the hand-off prompts' when Task 11/12 edits knowledge/. |
| PM (test-line budget) | Task 7 | PM (test-line budget, extends the G3 note): the bundle now carries 2,374 test lines (find bundles/security-testing -name '*.test.mjs' excluding evals/) against spec §10's ≤2,000, with Task 8's verify.test.mjs and the bin/ E2E still to come; Task 7 adds 299. The bound needs a ruling (binding vs advisory) before Task 14 asserts anything about it. |
| PM (plan text) | Task 7 | PM (plan text): Task 7's Files line says 'bin/check-skill-dupes.mjs (three groups)' and the File-structure line 'across two'; the branch correctly ships four three-way groups (engagement, cli, redact, redaction-rules — one file per group is the file's existing shape) per the G4 log. Amend lines 46/57/432 so the next reader does not flag the count. |
| Task 8 (one-liner, optional; PM may promote) | Task 8 | Task 8: [verify.mjs:233,240 → owner Task 8 (one-liner, optional; PM may promote — same class as the Task 6 render/status nit)] `decide` validates `finding_id`, `head`, `diff` and `tests` of the read-back verify.json but not `base`, and the VERDICT line is built with `ctx.out` (unredacted by design, from validated hex). A hand-edited `base` prints raw: reproduced on 501a9d7 with base=`password = hunter22xyz` ⇒ stdout `VERDICT VERIFIED … base=password = hunter22xyz head=…` exit 0 (the written file is field-wise redacted, so the leak is stdout only, trigger is a hand edit of a script-owned ignored file). Fix verified green (8/8 verify tests): add `\|\| !OID.test(doc.base)` to the CorruptError condition at :233 ⇒ `CORRUPT .agents/security-testing/verify/<id8>-<head7>/verify.json` exit 5. |
| PM (ratify) | Task 8 | Task 8: [fixtures/verify/sleep.mjs → owner PM (ratify)] A 3-line fixture the plan's Task 8 does not name (`exit3.mjs` is); it sits under the File structure's `scripts/fixtures/{cite,verify}/...` and exists only to exercise `runTests`' timeout path (exit null, timed_out true), which the Review Checklist requires a test for. Accepted as within the named fixture dir; PM to ratify so the size-discipline audit is explicit. |
| PM note (size) | Task 8 | Task 8: [size → owner PM note] verify.mjs 318 lines (spec ~250, plan ~230), lib/verify-steps.mjs 190 (plan ~120; the reference's parseUnifiedDiff kept whole). Header comment + JSDoc, no unnamed module or reference; not blocking per the Task 5/6 precedent. |
| PM (test-line budget, binding vs advisory) | Task 8 | Task 8: [PM: test-line budget, binding vs advisory] `find bundles/security-testing -name '*.test.mjs' \| xargs wc -l` on 501a9d7 = 2,389 lines (verify.test.mjs 231 + verify-steps.test.mjs 83 added). The G4 log's 1,646 undercounted — the same count is 2,075 on the merge base 0a4bc12 — so spec §10's ≤2,000 was already exceeded before Task 8; Task 7's cases tests and the Task 9–12 prose tests will add more. Decision needed before Task 14. |
| Task 8 (optional) / Task 14 (E2E) | Task 8 | Task 8: [verify.mjs:51 top-level `await import` → owner Task 8 (optional) / Task 14 (E2E)] `existsSync(REGISTER_PATH)` guards absence only; a present-but-unimportable register.mjs (half-installed risk-register missing its lib/) kills verify.mjs with an unhandled import error before any result line. Wrap in try/catch and treat as not installed (print REGISTER: skipped) or let it surface as INTERNAL. Task 14's installed E2E should run verify.mjs from `.claude/skills/secure-code-review/scripts/` so the `../../risk-register/scripts/register.mjs` resolution is exercised in the installed shape, not only in-repo. |
| Task 9/12 (docs) | Task 8 | Task 8: [verify.mjs:316 + cli.mjs usageText → owner Task 9/12 docs] `verify.mjs --help` prints `usage: verify <command> [options]` with a `verify` sub-command the CLI never takes (main prepends it). Cosmetic; SKILL.md/README should show the real synopsis `verify.mjs --finding <id> --review <dir> --head <oid> [--assertion not-refound\|refound --by <session>]`. |
| Task 11 (docs, no code change) | Task 8 | Task 8: [verify.mjs:222-226 registerStep → owner Task 11 docs, no code change] register's `UsageError`/`RefusedError` from `fixed`/`regressed` are unreachable via the CLI (rowForFinding returns LIVE rows only; `fixed` is called only when status≠fixed and the table allows it from open\|regressed\|accepted; `regressed` only from fixed) — verified in transitions.mjs:36-37. If ever reached, register's UsageError is a different class instance than secure-code-review's, so it would print `INTERNAL UsageError` exit 1 AFTER verify.json is already written with the verdict; the re-run then says `already decided`. Docs: recover by a first invocation (restart), which is D2-consistent. |
| Task 9/12 (docs) | Task 8 | Task 8: [verify.mjs:234 restart semantics → owner Task 9/12 docs] `USAGE(verify: run <id8>-<head7> is already decided: <verdict> (run without --assertion to restart it))` — a restart re-runs the tests and rewrites PENDING-REVIEW, but the register row stays where the previous decision moved it (fixed); the next `refound` then regresses it. Correct per D2/D7; the reviewer/lead prose should state that a restart does not undo the register event. |
| Task 2 (already logged after G4) | Task 8 | Task 8: [verify.mjs:174,152 redaction inheritance → owner Task 2 (already logged after G4)] `review`, `carried_dirt[]`, `diff.advisories[].path`, `tests.argv[]` go through `clean` on write (correct per the Global Constraint), so a ≥40-char alnum/slash path is eaten by the high-entropy rule exactly as logged for cite.mjs; `finding_id`/`base`/`head` are kept via HEX_KEYS and the VERDICT/CORRUPT lines are built from validated hex. Nothing for Task 8 to do; the rule fix must land before Task 7 pins the copies. |
| Task 8 (optional) | Task 8 | Task 8: [verify.test.mjs → owner Task 8 (optional)] The CORRUPT exit-5 path of `decide` (non-JSON / mismatched verify.json) is untested; verified manually (`CORRUPT .agents/security-testing/verify/<name>/verify.json`, exit 5). Three lines (write `not json` over verify.json, run the second invocation, assert code 5 and the line) would pin it beside the `already decided` assertion at :117-119. |
| Task 8 (cosmetic) | Task 8 | Task 8: [verify.mjs:279 → owner Task 8 (cosmetic)] `carried = dirtOf(root, [])` (a full `git status --porcelain -z`) runs on the second invocation too but `decide` never uses it; hoist into `firstRun` or gate on `assertion === undefined`. |
| Task 9 (docs) | Task 8 | Task 8: [verify.mjs:147 dirtOf + `--untracked-files=all` → owner Task 9 docs] When a consumer has not run `cite.mjs init` (no managed ignore block), `.agents/security-testing/reviews/<dir>/findings.json` and, on re-runs, the run dir itself appear in `carried_dirt` (recorded, never refused — correct). Docs: run `cite.mjs init` first so verify.json's carried_dirt lists only the operator's own dirt. |
| PM (accepted as spec-consistent) | Task 8 | Task 8: [dev deviations → owner PM (accepted as spec-consistent)] Early verdicts on the FIRST invocation (UNVERIFIED-NO-TEST-SURFACE at step 3, UNVERIFIED-DELETION-ONLY at step 4, UNVERIFIED-TESTS-FAILED from a non-zero/null exit) — spec §6 places those tokens at steps 3–4 before step 5's PENDING-REVIEW, and plan Step 3 says 'write verify.json with PENDING-REVIEW (or an early UNVERIFIED-*)', so this is plan-as-written not a deviation; the second invocation re-derives in the plan's order and refuses a decided run. Also accepted: `checkArgv` throws ArgvError (mapped to `USAGE(verify: execute_project_tests.argv rejected: <why>)`, verified for `-e` and `bash`); optional `execute_project_tests.timeout_s` (default 600, `"1"` string ⇒ USAGE, verified); timeout ⇒ tests.exit null + `[timed out after N s]` in tests.log ⇒ UNVERIFIED-TESTS-FAILED (verified end-to-end, plan's `tests` shape unchanged); `--review` cwd-resolved, stored root-relative posix, must be a directory under the tree (`../` and a file both refused, verified); one DIRTY line per path; no subcommand word with --help pass-through. |
| Task 13 (already logged) / PM | Task 8 | Task 8: [validators] node --test on 501a9d7 in a detached worktree: 874/874 pass (14 for Task 8's two files); validate:factories green; validate:dupes green (2 groups); the three generated marketplaces remain stale (Task 13, already logged); validate:externals not run per instruction. The task text's 'nothing merged' is stale — task/task-8 is based on 0a4bc12 which carries Tasks 1–6. |

### PM log additions (after G6)

Reviewer nits from Rio's PASS review of Task 9, recorded verbatim. Owner = the task that must act on the nit.

| Owner | Source | Nit |
|---|---|---|
| Task 9 (next touch) / Task 10 | Task 9 | Task 9: [Task 9 (next touch) / Task 10] bundles/security-testing/briefings/security-reviewer.md line 10: 'All git-ignored by the managed block cite.mjs init wrote' is false for the mitigation-review output `.agents/security-testing/second-M-nnn.json` — lib/ignore-block.mjs PATTERNS cover only reviews/, verify/, register/, proposals/. Reword to 'the review and verify directories are git-ignored; second-M-nnn.json beside threat-model.json is not', or Task 10 relocates the file when it fixes the threat-model flow. |
| Task 12 / next touch | Task 9 | Task 9: [Task 12 / next touch] SKILL.md 'Fixing a FAILED citation' last sentence and references/findings-shape.md step 3: 'valid again on the next check when the stamped bytes are unchanged' is muddled — after a citation fix the stamped bytes normally change, so existing second opinions stay STALE-REVIEW and are redone; say that plainly. |
| Task 12 (docs) / Task 8 (optional) | Task 9 | Task 9: [Task 12 docs / Task 8 optional] verify.mjs never removes the run dir on restart (no rmSync), so a stale fix-review.json from a previous decided run sits beside a fresh PENDING-REVIEW verify.json; lead prose should say a restart needs a fresh fix-review dispatch (the agent overwrites the file), or verify.mjs deletes fix-review.json when it rewrites PENDING-REVIEW. |
| Task 14 / PM log | Task 9 | Task 9: [Task 14 / PM log] evals/prompt.md and evals/README.md still say 'packet' (frozen harness reused as is per spec §5; prompt.md is inside the digest). prose.test.mjs's banned-vocabulary sweep deliberately excludes evals/ — record that in the PM log so no later task 'fixes' the wording and breaks prompt_sha256. |
| Task 14 | Task 9 | Task 9: [Task 14] prose.test.mjs 'standalone install' test never removes its mkdtempSync dir (same pattern as the script tests; cosmetic). |
| Task 9 (optional wording) | Task 9 | Task 9: [Task 9 optional wording] AGENT.md 'Reading at a commit': `cite.mjs show <path>` with no range prints the whole file — the 40-line refusal applies only to an explicit window. Harmless (the cap governs citations), but 'read a long file in consecutive windows' reads as if show enforces it. |
| Task 8 (cosmetic, already routed by dev) | Task 9 | Task 9: [Task 8 cosmetic, already routed by dev] verify.mjs --help prints a `verify` sub-command the CLI never takes; SKILL.md correctly shows the real synopsis beside the verbatim paste. |
| PM (budget row) | Task 9 | Task 9: [PM budget row] bundle *.test.mjs lines after Task 9: +374 (prose.test.mjs 180, score-findings.test.mjs 194) on top of the 2,389 counted at 501a9d7; spec §10's ≤2,000 remains exceeded — PM decision still open before Task 14. |
| Task 10/12 (wording) | Task 9 | Task 9: [Task 10/12 wording] taxonomy.md keeps `unmapped` as the 15th id (reserved for v2 SARIF import; reviewer never writes it) and notes cite.mjs only requires a non-empty class string — consistent with cite.mjs:484; Task 10/12 prose should not imply the script enforces the list. |
