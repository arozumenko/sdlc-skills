...
A case unit enters the pipeline with its case: the upstream contract the
reviewer walks the code against, backed by execution evidence. A **technical
...
case, and without a written equivalent the reviewer has nothing objective to
tick and the gate has no defined run set. The **brief** is that equivalent:
one markdown file per unit, written before the build dispatch, gated by the
orchestrator the way a case's coverage is gated.
...
Where a case unit is grounded in *execution evidence*, the brief is grounded
...
same commit-what-you-produce rule as the spec and the surface cache: the plan
survives an interrupted run, and the reviewer reads it from disk, not from
your context.

## Builder substitutions — how the six-phase loop absorbs a brief

The builder slot (`test-automation-implementation` skill) runs its normal
loop on a brief with three substitutions:

- **Phase 1** absorbs the brief (scope, out-of-scope, acceptance criteria)
  instead of a case's steps — a brief missing a required section goes
  back to the orchestrator naming the section, the same move as returning an
  ungroundable case.
...
the case would go, and adjacent debt spotted mid-build returns as a
finding, never as silent scope widening.

## Required sections

A brief missing any of these is `blocked`, not dispatchable — the mirror of
the coverage contract's "no silent gap" rule.
...
4. **Acceptance criteria** — at least one checkable statement per unit. These
   are what the reviewer ticks (the brief sits where the case sits in the
   reviewer's walk: source item ↔ brief ↔ diff) and what the close sweep
...
new or changed specs. The report uses the same outcome vocabulary
(`delivered` reads "proven and landed"), and the close sweep back-writes the
