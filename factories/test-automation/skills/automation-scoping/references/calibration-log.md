...
Factory maintainers: append a dated entry here whenever the shipped
...
Entries below describe the pipeline **as it ran when measured**. Those dated
before the v2 rework reference roles and artifacts that no longer exist
(`qa-engineer`/analyst dispatches, AFS spec files under `test-specs/`) —
they are the calibration projects' historical record, not the current
pipeline's shape.
...
efficiency-audit output (not disclosable — this factory is public).
...
first per-case efficiency report this factory drew on). `findCaseSnapshot`
