...
test('deriveSize: a frontmatter size (test-sizer output) beats the derived rubric', () => {
  const s = deriveSize(taxonomy, {
    steps: 3, tier: { multiplier: 1.0 },
    verdict: { surfaces: 1, new_abstractions: 0 },
    frontmatterSize: 'L',
  });
  assert.equal(s.size, 'L');
  assert.equal(s.sp, 8);
  assert.equal(s.basis, 'frontmatter');
});

test('deriveSize: a reader verdict still overrides a frontmatter size', () => {
  const s = deriveSize(taxonomy, {
    steps: 3, tier: { multiplier: 1.0 },
    verdict: { size: 'S', surfaces: 1, new_abstractions: 0 },
    frontmatterSize: 'L',
  });
  assert.equal(s.size, 'S');
  assert.equal(s.basis, 'verdict');
...
test('deriveSize: an off-scale frontmatter size falls through to the rubric', () => {
  const s = deriveSize(taxonomy, {
    steps: 3, tier: { multiplier: 1.0 },
    verdict: { surfaces: 1, new_abstractions: 0 },
    frontmatterSize: 'XXL',
...
test('scoreCase: accepts size: from TC frontmatter, case-normalized', () => {
  const dir = mkdtempSync(join(tmpdir(), 'scoping-test-'));
  const file = join(dir, 'TC-9.md');
  writeFileSync(file, `---
id: TC-9
size: l
---
...
| 2 | Save | persists |
`);
  const [row] = loadCases(file);
  const s = scoreCase(row, taxonomy);
  assert.equal(s.size, 'L');
  assert.equal(s.sizeBasis, 'frontmatter');
  rmSync(dir, { recursive: true, force: true });
});
