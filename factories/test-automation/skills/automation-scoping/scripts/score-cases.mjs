...
// the flat `key: value` shape every TMS-case template in this family
...
// references/sizing-rubric.md. A reader's verdict.size always wins, then a
// case's own frontmatter `size:` (test-sizer's rating); the driver-score
...
export function deriveSize(taxonomy, { steps, tier, verdict, frontmatterSize } = {}) {
  ...
  // 2. A frontmatter `size:` (manual-qa's test-sizer already rated this
  //    case) beats the derived fallback — never re-derive what's rated.
  //    A reader's verdict still overrides it (rule 1).
  const fmSize = frontmatterSize ? String(frontmatterSize).trim().toUpperCase() : null;
  if (fmSize && scale.points[fmSize] !== undefined) {
    return sizeOf(fmSize, 'frontmatter', { points: null, breakdown: {} });
  }
...
export function scoreCase(caseRow, taxonomy, opts = {}) {
  ...
  const sized = deriveSize(taxonomy, {
    steps, tier, verdict, frontmatterSize: caseRow.frontmatter?.size,
  });
